...
Public Class Halaman_Edit_Admin
    ...
    Function cekJudul() As Boolean
        ...
        Try
            conn.Open()
            Dim query As String = "SELECT judul FROM film WHERE judul = @ju AND @ju != @ju_lama"
            cmd = New MySqlCommand(query, conn)
            cmd.Parameters.AddWithValue("@ju", txtJudul.Text)
            cmd.Parameters.AddWithValue("@ju_lama", judulLama)
            reader = cmd.ExecuteReader()
    ...
    Private Sub btnEdit_Click(sender As Object, e As EventArgs) Handles btnEdit.Click
        ...
        Else
            ...
            Try
                conn.Open()
                Dim query As String = "UPDATE film SET judul = @ju, genre = @ge, tglrilis = @tr, rating = @ra, gambar = @ga WHERE judul = @ju_lama AND id_akun = @id"
                cmd = New MySqlCommand(query, conn)
                cmd.Parameters.AddWithValue("@ju", txtJudul.Text)
                ...
                Dim gambarPath As String = PictureBox1.ImageLocation
                cmd.Parameters.AddWithValue("@ga", gambarPath)
                cmd.Parameters.AddWithValue("@id", sesiAdmin)
                cmd.Parameters.AddWithValue("@ju_lama", judulLama)
                cmd.ExecuteNonQuery()
                MessageBox.Show("Berhasil Mengedit Film")
            Catch ex As Exception
                MessageBox.Show("Error: " & ex.Message)
            Finally
                conn.Close()
